...
def next_func(update, context):
    ...
    try:
        ...
        if stage_== 3:
            ...
            connect.commit()
            context.bot.send_message(text=dct[lang_][9], chat_id=user_id)
            sleep(1)
...
def biginner(update, context):
    ...
    if stage_ == 6:
        ...
        sleep(1)
        context.bot.send_message(text=dct[lang_][7].format(name, num_, select_level_, cost), chat_id=957531477)
        sleep(1)
        cur.execute(stagee.format('{}', user_id).format(7)).fetchall()
        connect.commit()
        stag = cur.execute(stage.format(user_id)).fetchall()
        connect.commit()
        stag = stag[0][0]

        if stag == 7:
            keyboard = [
                InlineKeyboardButton(text='🧐🧐🧐', callback_data='Avazbek')
            ]
            context.bot.send_message(chat_id=user_id, text=dct[lang_][8], reply_markup=InlineKeyboardMarkup([keyboard]))
            sleep(1)
...
def Avazbek(update,context):
    ...
    lang_ = lang_[0][0]
    text = dct[lang_][10]
    context.bot.send_photo(chat_id=user_id, photo=open('Photo_base/Teacher.JPG', 'rb'), caption=text )
